...
static void bfc_kmer_insert(cnt_step_t *cs, const bfc_kmer_t *x, int is_high, int tid)
{
        int k = cs->k;
	uint64_t y[2];
	/* bfc_kmer_hash populates y[]; the return value is unused here. */
	(void) bfc_kmer_hash(k, x->x, y);
...
struct bfc_ch_s *fml_count(int n, const fseq1_t *seq, int k, int q, int l_pre, int n_threads)
{
	int i;
	cnt_step_t cs;
